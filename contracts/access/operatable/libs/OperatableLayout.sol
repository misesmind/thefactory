...
struct OperatableStruct {
    mapping(address => bool) isOperator;
}

library OperatableLayout {
     ...
     */
    function slot(
        OperatableStruct storage table
    ...
    function layout(
        bytes32 slot_
    ) external pure returns(OperatableStruct storage layout_) {
    ...
    function _layout(
        bytes32 slot_
    ) internal pure returns(OperatableStruct storage layout_) {
